...
- Auto-closes terminal buffers when Vim exits — `QuitPre` autocommand gated on the last non-terminal window, so a single-window or aborted `:q` never kills terminals, and `:qall` never trips `E947` (job still running)
