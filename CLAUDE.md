...
- `terminal.bashrc` exports `SHELL_SESSION_DID_INIT=1` before sourcing `/etc/profile` so macOS's Apple-Terminal session save/restore (`/etc/bashrc_Apple_Terminal`) is skipped — it's the only `date` call in the terminal and prints a noisy `Restored session:` line on every open. No-op on Linux
...
Both installers back up existing `~/.vim` and `~/.vimrc` with timestamps, verify symlink targets before removing during uninstall, refuse to run as root, and support `--help/-h`. `install.sh` creates the undo dir at `~/.local/share/vim/undodir` with `chmod 700`. `get-ivim.sh` clones to `~/.ivim` and checks out a **pinned release commit** (`IVIM_VERSION` / `IVIM_COMMIT` at the top of the script — currently `0.1.7` = `14fa639`), not `master` HEAD: the curl'd script is always latest-from-master, but the installed config is the pinned, integrity-verified commit (detached HEAD; bump both vars to ship a new version). Updates `git fetch` + re-checkout the pin, aborting on a mismatched remote or a failed integrity check, and clean up a failed clone. Its `--uninstall` dirty-guard treats HEAD as safe to remove only when every commit is present on `origin` (`rev-list --count HEAD --not --remotes=origin` = 0) — so a pinned detached HEAD removes cleanly while genuine local commits are protected — then deletes `~/.ivim`, whereas `install.sh --uninstall` never touches the source directory.
